...
export namespace BuyerSigner {
  ...
  export async function selectPaymentUTXOs(
    ...
    if (selectedAmount < amount) {
      throw new InsufficientFundsError(`Not enough cardinal spendable funds.
...
Needed:       ${satToBtc(amount)} BTC`);
  ...
  export async function generateUnsignedBuyingPSBTBase64(
    ...
    const recommendedFeeRate = await getFees(listing.buyer.feeRateTier);
    const fee = calculateFeeWithRate(543.75, recommendedFeeRate)
    ...
    if (changeValue < 0) {
      throw new InsufficientFundsError(`Your wallet address doesn't have enough funds to buy this inscription.
...
Missing:    ${satToBtc(-changeValue)} BTC`);
  ...
  export async function generateUnsignedCreateDummyUtxoPSBTBase64(
    ...
    const finalFees = calculateFeeWithRate(197.5,  recommendedFee)
