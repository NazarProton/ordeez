...
(function (BuyerSigner) {
    ...
    vinsLength, voutsLength, feeRateTier, itemProvider) {
        ...
        if (selectedAmount < amount) {
            throw new interfaces_1.InsufficientFundsError(`Not enough cardinal spendable funds.
...
Needed:       ${(0, util_1.satToBtc)(amount)} BTC`);
    ...
    async function generateUnsignedBuyingPSBTBase64(listing) {
        ...
        for (const dummyUtxo of listing.buyer.buyerDummyUTXOs) {
            ...
            signInputIndexes.push(currentInputIndex);
            currentInputIndex++;
        }
        // Add dummy output
        psbt.addOutput({
        ...
        for (const utxo of listing.buyer.buyerPaymentUTXOs) {
            ...
            signInputIndexes.push(currentInputIndex);
            currentInputIndex++;
            if (listing.buyer.buyerPaymentUTXOs.length > 1) {
                await sleep(1000);
            }
        }
        ...
        const recommendedFeeRate = await (0, mempool_1.getFees)(listing.buyer.feeRateTier);
        const fee = (0, feeprovider_1.calculateFeeWithRate)(543.75, recommendedFeeRate);
        ...
        if (changeValue < 0) {
            throw new interfaces_1.InsufficientFundsError(`Your wallet address doesn't have enough funds to buy this inscription.
...
Missing:    ${(0, util_1.satToBtc)(-changeValue)} BTC`);
        ...
        listing.buyer.unsignedBuyingPSBTSigningIndexes = signInputIndexes;
        return listing;
    }
    ...
    async function generateUnsignedCreateDummyUtxoPSBTBase64(address, buyerPublicKey, unqualifiedUtxos, feeRateTier, itemProvider) {
        ...
        const finalFees = (0, feeprovider_1.calculateFeeWithRate)(197.5, recommendedFee);
        ...
        if (changeValue < 0) {
            throw new interfaces_1.InsufficientFundsError(`You might have pending transactions or not enough fund`);
        }
        psbt.addOutput({
            address,
            value: constant_1.DUMMY_UTXO_VALUE,
