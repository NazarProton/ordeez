...
const storageInitialize = require('./services/storageInitialize');
...
const init = async () => {
  ...
  if(process.env.NODE_ENV === 'development') {
    app.server.listen(config.app.port, async () => {
      await logger.info(`[server] Server listening at port ${config.app.port}`);
    });
  } else {
    app.server.listen(config.app.port, async () => {
      await logger.info(`[server] SSL server is running at ${config.app.port}`);
    });
  }
};
